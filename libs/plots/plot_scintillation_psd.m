...
for constellation = constellations
    ...
    for i = 1:numel(out.(constellation).scenario)
        ...
        for j = 1:numel(freq_names)
            ...
            nexttile(j);
            hold on;
            plot(mu_pos, 10*log10(Imu_est), 'LineWidth', 1.2);
            plot(mu_pos, 10*log10(Imu_theory), 'r--', 'LineWidth', 1.2);
            xline(spectral_params.mu0, '--', 'LineWidth', 1.0, ...
            ...
            plot(mu_pos, 10*log10(phase_P_est_refractive), 'LineWidth', 1.2);
            plot(mu_pos, 10*log10(phase_P_est_total), 'LineWidth', 1.2);
            plot(mu_pos, 10*log10(phase_P_theory), 'LineWidth', 1.2);
            xline(spectral_params.mu0, '--', 'LineWidth', 1.0, ...
                'Color', [0.85 0.85 0.85], 'HandleVisibility', 'off');
            set(gca, 'XScale', 'log', 'FontName', 'Helvetica', 'FontSize', font_axis);
            xlim(xlim_mu);
